    ...
    try:
        ...
        for su_qq in super_id:
            await bot.send_private_msg(user_id=int(su_qq),
                                        message=f"QQ：{add_qq} 请求添加bot为好友!\n请求添加时间：{realtime}\n验证信息为：{comment}")
    except Exception as e:
        for su_qq in super_id:
            await bot.send_private_msg(user_id=int(su_qq), message=f"罐头坏掉了\n错误信息：{e}")
...
async def _(bot: Bot, event: PrivateMessageEvent):
    try:
        ...
        if agree_id in qq_set:
            await bot.send_private_msg(user_id=user_id, message=f"bot成功添加QQ:{agree_id}为好友！")
        ...
        for su_qq in super_id:
            await bot.send_private_msg(user_id=int(su_qq), message=f"罐头出错了\n错误信息：{e}")